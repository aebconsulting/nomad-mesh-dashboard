...
export function Vitals({ status, stats, nodes }: { status: Status | null; stats: StatsData | null; nodes: Node[] }) {
  ...
  return (
    <div className="vitals" role="status" aria-label="Mesh vitals">
      ...
      <div className="vital"><span className="k">Snapshot</span><span className={snapAge != null && snapAge > 300 ? "v down" : "v"}>{snapAge == null ? "—" : age(snapAge)}</span><span className="sub">node data age</span></div>
    </div>
  );
}
