...
- **Phase 5 — AI mesh-analyst chat (Aaron, 2026-07-11):** an in-dashboard AI helper that can SEE the live mesh stats (vitals, node health, telemetry summaries) and explain what they mean in plain language ("why is this link weak?", "is the mesh healthy?"). Per Aaron: it uses the MAIN chat assistant — the same @ai brain the bridge already runs (same model/config/memory, one assistant identity everywhere). Shape: Meridian backend packages current `/api/nodes`+`/api/stats`+vitals context and asks that same brain; CONSTRAINT to settle in the brainstorm: dashboard-originated questions must NOT transit the radio (no RF airtime for web-only chat) — so either a bridge non-RF ask endpoint or calling the bridge's own LLM_BASE_URL with the bridge's system persona. Read-only by design. Needs its own brainstorm + plan (context packaging, streaming, guardrails against presenting inference as mesh fact on a life-safety system).
